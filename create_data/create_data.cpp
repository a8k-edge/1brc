...
public:
    WeatherStation(const std::string& id, double meanTemperature)
        : id(id), meanTemperature(static_cast<int>(meanTemperature)) {}

    void measurement(std::ostringstream& buffer) {
        int m = meanTemperature + (dist(gen) - 10);
        char d = '0' + dist(gen) % 10;
...
void produce(std::ofstream& file, std::vector<WeatherStation>& stations, int count) {
    std::ostringstream buffer;

    for (int i = 0; i < count; ++i) {
        stations[rand() % stations.size()].measurement(buffer);

        if (i % 1000 == 0 || i == count - 1) { // Flush every 1000 measurements or at the end
            file << buffer.str();
...
int main() {
    ...
    std::vector<WeatherStation> stations = {
        ...
        WeatherStation("Portland WeatherStation(OR)", 12.4),
    ...
    int size = 1000000000; // Number of records

    clock_t start = clock();
    produce(file, stations, size);
    clock_t end = clock();

    std::cout << "Created file with " << size << " measurements in " << double(end - start) / CLOCKS_PER_SEC << " seconds\n";
